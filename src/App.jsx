...
import { useRounder } from "./themeAlgorithms/rounder";
const { useToken } = theme;
function App() {
  const [dark, setDark] = useState(false);
  const [square, seSquare] = useState(false);
  const handleChecked = (checked) => {
    if (checked) setDark(true);
    else setDark(false);
  };
  const { token } = useToken();
  const [squareAlgorithm, rounderAlgorithm] = useRounder(
    dark ? darkTheme : lightTheme
  );
  return (
    <ConfigProvider
      theme={{
        algorithm: square ? squareAlgorithm : "",
      ...
      <Layout
        ...
        <Space direction="vertical" size={10}>
          ...
          <Button onClick={() => seSquare((x) => !x)}>Border Change</Button>
